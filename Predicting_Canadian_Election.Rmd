---
title: "The Impact of Voters' Demographic Factors in Canadian Federal Elections"
author: "Jenny Oh"
...
survey_data <- read_csv("survey_cleaned.csv")
```


```{r, eval = F, include=F}
## DATA CLEANING
census_data_cleaned <- census_data %>% select(age, sex, income_family,province, 
                                              education, religion_has_affiliation, marital_status,
                                              language_knowledge, occupation)

census_cleaned <- census_data_cleaned %>% filter(!is.na(education)) %>% 
  mutate(age = round(age, digits = 0)) %>% 
  mutate(education = replace (education, education == "Bachelor's degree (e.g. B.A., B.Sc., LL.B.)", "Bachelor's degree")) %>% mutate(education = replace (education, education == "College, CEGEP or other non-university certificate or di...", "College or certificate")) %>% 
  mutate(education = replace (education, education == "High school diploma or a high school equivalency certificate", "High school diploma")) %>% 
  mutate(education = replace (education, education == "Less than high school diploma or its equivalent", "Less than high school")) %>% 
  mutate(education = replace (education, education == "University certificate or diploma below the bachelor's level", "College or certificate")) %>% 
  mutate(education = replace (education, education == "Trade certificate or diploma", "College or certificate")) %>% 
  mutate(education = replace (education, education == "University certificate, diploma or degree above the bach...", "Above bachelor")) %>%
  filter(!is.na(religion_has_affiliation)) %>% 
  mutate(religion_has_affiliation = replace (religion_has_affiliation, religion_has_affiliation == "Don't know", "No")) %>%
  mutate(religion_has_affiliation = replace (religion_has_affiliation, religion_has_affiliation == "Has religious affiliation", "Yes")) %>% 
  mutate(religion_has_affiliation = replace (religion_has_affiliation, religion_has_affiliation == "No religious affiliation", "No")) %>% # are you religious  
  rename(religious = religion_has_affiliation) %>% 
  filter(!is.na(marital_status)) %>%
  mutate(marital_status = replace(marital_status, marital_status == "Divorced", "Single")) %>% 
  mutate(marital_status = replace(marital_status, marital_status == "Living common-law", "Has a partner")) %>% 
  mutate(marital_status = replace(marital_status, marital_status == "Married", "Has a partner")) %>% 
  mutate(marital_status = replace(marital_status, marital_status == "Separated", "Single")) %>% 
  mutate(marital_status = replace(marital_status, marital_status == "Single, never married", "Single")) %>%
  mutate(marital_status = replace(marital_status, marital_status == "Widowed", "Single")) %>% 
  filter(!is.na(language_knowledge)) %>%
  filter(language_knowledge != "Don't know") %>% 
  mutate(occupation = replace(occupation, occupation == "Business, finance, and administration occupations", "Yes")) %>%
  mutate(occupation = replace(occupation, occupation == "Health occupations", "Yes")) %>%
  mutate(occupation = replace(occupation, occupation == "Management occupations", "Yes")) %>%
  mutate(occupation = replace(occupation, occupation == "Natural and applied sciences and related occupations", "Yes")) %>%
  mutate(occupation = replace(occupation, occupation == "Natural resources, agriculture and related production oc...", "Yes")) %>%
  mutate(occupation = replace(occupation, occupation == "Occupations in art, culture, recreation and sport", "Yes")) %>%
  mutate(occupation = replace(occupation, occupation == "Occupations in education, law and social, community and ...", "Yes")) %>%
  mutate(occupation = replace(occupation, occupation == "Occupations in manufacturing and utilities", "Yes")) %>%
  mutate(occupation = replace(occupation, occupation == "Sales and service occupations", "Yes")) %>%
  mutate(occupation = replace(occupation, occupation == "Trades, transport and equipment operators and related oc...", "Yes")) %>%
  mutate(occupation = replace(occupation, occupation == "Uncodable", "Yes")) %>%
  mutate(occupation = replace_na(occupation, "No")) %>% 
  rename(employed = occupation) %>%
  select(age, sex, income_family, province, education, religious, marital_status, language_knowledge, employed)
  
  # no NAs for age, sex, income_household, and province
  # province : currently living in but no territories 
  # education: remove NAs

write_csv(census_cleaned, "census_cleaned.csv")
...
# survey_var <- tibble(names(survey_data))

get_decon()
names_decon <- tibble(names(decon))

decon_cleaned <- decon %>% filter(citizenship == "Canadian citizen")

decon_cleaned <- decon_cleaned %>% select(yob, gender, income, province_territory, education, religion, marital,
                                          language_eng, language_fr, language_abgl, employment, votechoice)

survey_cleaned <- decon_cleaned %>% 
  filter(!is.na(yob)) %>%
  mutate(age = 2019-as.numeric(as.character(yob))) %>% 
  mutate(gender = replace(as.character(gender), as.character(gender) == "A man", "Male")) %>% # assume male = man and female = woman
  mutate(gender = replace(as.character(gender), as.character(gender) == "A woman", "Female")) %>% 
  filter(as.character(gender) == "Female" | as.character(gender) == "Male") %>% 
  rename(sex =gender) %>% 
  filter(!is.na(income)) %>% 
  mutate(income_family = case_when(income >= 100000 & income < 125000 ~"$100,000 to $ 124,999",
                                   income >= 125000 ~ "$125,000 and more",
                                   income >= 25000 & income < 50000 ~ "$25,000 to $49,999",
                                   income >= 50000 & income < 75000 ~ "$50,000 to $74,999",
                                   income >= 75000 & income < 1000000 ~"$75,000 to $99,999",
                                   income < 25000~ "Less than $25,000")) %>% 
  filter(province_territory!= "Northwest Territories" & province_territory!= "Nunavut" & province_territory!= "Yukon") %>% 
  ...
  mutate(education = case_when(education == "No schooling" | education =="Some elementary school" | education =="Completed elementary school" | education =="Some secondary/ high school" ~ "Less than high school",
                               education =="Completed secondary/ high school" ~ "High school diploma",
                               education =="Some technical, community college, CEGEP, College Classique" | education =="Completed technical, community college, CEGEP, College Classique" | education =="Some university" ~ "College or certificate",
                               education =="Bachelor's degree" ~ "Bachelor's degree",
                               education =="Master's degree" | education =="Professional degree or doctorate" ~ "Above bachelor")) %>%  filter(!is.na(education)) %>% 
  mutate(religious = case_when(religion == "None/ Don't have one/Atheist" | religion == "Don't know/ Prefer not to answer" ~ "No",
                               religion != "None/ Don't have one/Atheist" & religion != "Don't know/ Prefer not to answer" ~ "Yes")) %>% filter(!is.na(marital)) %>%
  filter(marital != "Don't know/ Prefer not to answer") %>% 
  mutate(marital_status = case_when(marital == "Married" | marital == "Living with a partner" ~ "Has a partner",
                                    !(marital == "Married" | marital == "Living with a partner") ~ "Single")) %>% 
  mutate(language_knowledge = case_when(language_eng == "English" & language_fr == "French" ~ "Both English and French",
                                        language_eng == "English" & is.na(language_fr) ~ "English only",
                                        is.na(language_eng)  & language_fr == "French" ~ "French only",
                                         is.na(language_eng) & is.na(language_fr) ~ "Neither English nor French")) %>% 
  mutate(employed = case_when(employment == "Working for pay full-time" | employment == "Working for pay part-time" | employment == "Self employed (with or without employees)" ~ "Yes",
                              !(employment == "Working for pay full-time" | employment == "Working for pay part-time" | employment == "Self employed (with or without employees)") ~ "No")) %>% 
  mutate(vote_liberal = case_when(votechoice == "Liberal Party" ~ 1, 
                                votechoice != "Liberal Party" ~ 0)) %>% 
  filter(!is.na(votechoice)) %>% 
  select(age,	sex,	income_family,	province,	education,	religious,	marital_status,	language_knowledge,	employed, votechoice, vote_liberal) 

write_csv(survey_cleaned, "survey_cleaned.csv")
```
...
```{r, echo = F,fig.align = 'center', fig.dim = c(4, 3), fig.cap="Histograms of age distributions in census data and survey data."}
# Use this to create some plots. Should probably describe both the sample and population.
c_age <- census_data %>% ggplot(aes(x = age, fill = cut(age, 100))) + geom_histogram(show.legend = FALSE) + labs(x = "age") + ggtitle("Age Distribution of the Census Data")
s_age <- survey_data %>% ggplot(aes(x = age, fill = cut(age, 100))) + geom_histogram(show.legend = FALSE) + labs(x = "age") + ggtitle("Age Distribution of the Survey Data")

grid.arrange(c_age + scale_fill_discrete(h = c(180, 360), c = 150, l = 80), s_age + scale_fill_discrete(h = c(240, 10), c = 120, l = 70))
```
...
```{r, echo = F, fig.dim = c(8, 3), fig.align = 'center', fig.cap="Bar plot of voting for the Liberal Party based on sex."}
ggplot(survey_data, 
       aes(x = sex, 
           fill = vote_liberal_char)) + geom_bar() + labs(x = "What is your sex?") + ggtitle("Voting for the Liberal Party based on sex") + scale_fill_discrete(name = "Vote for Liberal Party?")
```



```{r, echo = F}
table_sex <- table(survey_data$vote_liberal_char, survey_data$sex)
rownames(table_sex) = c("Vote for Liberal", "Not Vote for Liberal")
round(prop.table(table_sex, 2),3) %>% kable(caption = "Voting for the Liberal Party based on sex.")
```
...
```{r, echo = F, fig.dim = c(8, 3), fig.align='center', fig.cap="Bar plot of voting for the Liberal Party based on province."}
ggplot(survey_data, 
       aes(x = province, 
           fill = vote_liberal_char)) + geom_bar() + labs(x = "What Province Are You Living In?") + ggtitle("Voting for the Liberal Party based on province") + scale_fill_discrete(name = "Vote for Liberal Party?") + coord_flip()

table_province <- table(survey_data$vote_liberal, survey_data$province)
rownames(table_province) = c("Not Vote for Liberal", "Vote for Liberal") 
colnames(table_province) = c("AB", "BC", "MB", "NB", "NL", "NS", "ON", "PE", "QC", "SK")
round(prop.table(table_province, 2),3) %>% kable(caption = "Voting for the Liberal Party based on province.")
```
...
$$ log (\frac{P}{1-P}) = \beta_0+ \beta_1  x_{income} + \beta_2  x_{province} + \beta_3  x_{education} + \beta_4  x_{religious} + \beta_5  x_{language}$$

Here, $P$ represents the probability of an individual voting for the Liberal Party, the coefficients of $\beta_1$, $\beta_2$, $\beta_3$, $\beta_4$, and $\beta_5$ represent the change in log odds for one unit increase in $x_{income}$, $x_{province}$, $x_{education}$, $x_{religious}$, and $x_{language}$ respectively, and $\beta_0$ represents the intercept term.
...
Here, $\hat{y_j}$ is the cell-level estimate, that is the probability that a person with the specified demographic factors in this subgroup of the population will vote for the Liberal Party, using the logistic model we built. $N_j$ is the size of the $j^{th}$ subgroup of the census data. We calculated $\hat{y}^{PS}$, which is the probability that an individual in the specified subgroup votes for the Liberal Party, adjusted for the target population and not the sample.

```{r, include = F}

# Creating the Model
model <- glm(vote_liberal ~ income_family + province + education +
           language_knowledge + religious, data= survey_data, family = binomial)
...
To implement these two outlined methods, we gathered the census data counts, as the population cell count, after grouping by the five demographic factors we outlined earlier. The logistic regression model was then used to calculate the estimates, which was stated earlier to be the probability that someone from the subgroup specified by a row in our table would vote for the Liberaly Party. These estimates were then corrected for with poststratification by accounting for the census data counts and our five demographic factors and the data was put into a poststratification table that includes values for each of the five demographic factors and the corresponding probability of someone with those demographic factors voting for the Liberal Party. A section of this table can be seen in the next section, as well as in the Appendix.
...
census_data_counts <- census_data %>% 
  group_by(income_family, province, education, religious, language_knowledge) %>% 
...
post_stra_table <- census_data_counts %>% 
  mutate(liberal_predict_prop = estimate*n) %>%
  summarise(liberal_predict = sum(liberal_predict_prop)/sum(n),.groups = 'keep')  %>% arrange(desc(liberal_predict))

colnames(post_stra_table) = c("Household Income", "Province", "Education", "Religious", "Language", "Probability of Voting for Liberal") 

# post_stra_table %>% kable()

post_stra_table %>% head(n=10) %>%  kable(caption = "Poststratification Table (in decscending order of proability of voting for the Liberal Party)")
```
...
2. Voting Behaviour in Canada | The Canadian Encyclopedia. (n.d.). Www.thecanadianencyclopedia.ca. https://www.thecanadianencyclopedia.ca/en/article/electoral-behaviour 
...
4. Canada, S. (2020). General Social Survey Cycle 31: Family, 2017. Abacus.library.ubc.ca. https://abacus.library.ubc.ca/dataset.xhtml?persistentId=hdl:11272.1/AB2/G3DUFG 
...
6. Stephenson, Laura B; Harell, Allison; Rubenson, Daniel; Loewen, Peter John, 2020, "2019 Canadian Election Study (CES) - Online Survey Codebook",
https://doi.org/10.7910/DVN/DUS88V, Harvard Dataverse, V1.

7. Canada, S. (2020). General Social Survey Cycle 31: Family, 2017. Abacus.library.ubc.ca. “User’s Guide”, https://abacus.library.ubc.ca/dataset.xhtml?persistentId=hdl:11272.1/AB2/G3DUFG 
...
14. Grolemund, G. (2014, July 16) *Introduction to R Markdown*. RStudio. [https://rmarkdown.rstudio.com/articles_intro.html](https://rmarkdown.rstudio.com/articles_intro.html). (Last Accessed: January 15, 2021) 
...
16. Allaire, J.J., et. el. *References: Introduction to R Markdown*. RStudio. [https://rmarkdown.rstudio.com/docs/](https://rmarkdown.rstudio.com/docs/). (Last Accessed: January 15, 2021)


\newpage
## Appendix

```{r, echo=F, fig.dim = c(8, 3), fig.align='center', fig.cap="Bar plot of voting for the Liberal Party based on family income."}
ggplot(survey_data, 
       aes(x = income_family, 
           fill = vote_liberal_char)) + geom_bar() + labs(x = "How much is your family income?") + ggtitle("Bar Plot: Voting for Liberal Party based on income") + scale_fill_discrete(name = "Vote for Liberal Party?") + coord_flip()

table_income <- table(survey_data$vote_liberal, survey_data$income_family)
rownames(table_income) = c("Not Vote for Liberal", "Vote for Liberal") 
#colnames(table_province) = c("AB", "BC", "MB", "NB", "NL", "NS", "ON", "PE", "QC", "SK")
round(prop.table(table_income, 2),3) %>% kable(caption = "Voting for Liberal Party Based on the Income Factor")
```

```{r, echo=F, fig.dim = c(8, 3), fig.align='center', fig.cap="Bar plot of voting for the Liberal Party based on family education level."}
ggplot(survey_data, 
       aes(x = education, 
           fill = vote_liberal_char)) + geom_bar() + labs(x = "What is your highest education level?") + ggtitle("Bar Plot: Voting for Liberal Party based on education") + scale_fill_discrete(name = "Vote for Liberal Party?") + coord_flip()

table_education <- table(survey_data$vote_liberal, survey_data$education)
rownames(table_education) = c("Not Vote for Liberal", "Vote for Liberal") 
#colnames(table_province) = c("AB", "BC", "MB", "NB", "NL", "NS", "ON", "PE", "QC", "SK")
round(prop.table(table_education, 2),3) %>% kable(caption = "Voting for Liberal Party Based on the Education Factor")
```

```{r, echo=F, fig.dim = c(8, 3), fig.align='center', fig.cap="Bar plot of voting for the Liberal Party based on being religious or not."}
ggplot(survey_data, 
       aes(x = religious, 
           fill = vote_liberal_char)) + geom_bar() + labs(x = "Are you religioius?") + ggtitle("Bar Plot: Voting for Liberal Party based on Religion") + scale_fill_discrete(name = "Vote for Liberal Party?") + coord_flip()

table_religion <- table(survey_data$vote_liberal, survey_data$religious)
rownames(table_religion) = c("Not Vote for Liberal", "Vote for Liberal") 
colnames(table_religion) = c("Not Religious", "Religious")
round(prop.table(table_religion, 2),3) %>% kable(caption = "Voting for Liberal Party Based on the Religion Factor")
```

```{r, echo=F,fig.dim = c(8, 3), fig.align='center', fig.cap="Bar plot of voting for the Liberal Party based on knowledge of English, French, or both languages."}
ggplot(survey_data, 
       aes(x = language_knowledge, 
           fill = vote_liberal_char)) + geom_bar() + labs(x = "What Languages Do You Speak?") + ggtitle("Bar Plot: Voting for Liberal Party based on Language") + scale_fill_discrete(name = "Vote for Liberal Party?") + coord_flip()

table_language <- table(survey_data$vote_liberal, survey_data$language_knowledge)
rownames(table_language) = c("Not Vote for Liberal", "Vote for Liberal") 
#colnames(table_province) = c("AB", "BC", "MB", "NB", "NL", "NS", "ON", "PE", "QC", "SK")
round(prop.table(table_language, 2),3) %>% kable(caption = "Voting for Liberal Party Based on the Language Factor")
```

This shows the estimated $\hat{\beta_0}$,$\hat{\beta_1}$, $\hat{\beta_2}$, $\hat{\beta_3}$, $\hat{\beta_4}$, and $\hat{\beta_5}$ values.
```{r, echo = F}
...
```{r, echo =F}
post_stra_table2 <- census_data_counts %>% 
  mutate(liberal_predict_prop = estimate*n) %>%
  summarise(liberal_predict = sum(liberal_predict_prop)/sum(n),.groups = 'keep')


# post_stra_table %>% kable()

data_row <- post_stra_table2[sample(1:nrow(post_stra_table2)), ] 

colnames(post_stra_table) = c("Household Income", "Province", "Education", "Religious", "Language", "Probability of Voting for Liberal") 

data_row %>% head(n=25) %>%  kable(caption = "Post-stratification Table: Randomized Order")
```
